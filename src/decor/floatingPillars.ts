 ...
 * @param endX Where the pillar grid should stop on the X axis.
 ...
 * @param endZ Where the pillar grid should end on the Z axis.
...
function FloatingPillars(startBeat: number, endBeat: number, amountPerRow: number, spacing: number, startX: number, endX: number, height: number, width: number, startZ: number, endZ: number, track: string, R: number, G: number, B: number, A: number) {
    let z = startZ
